...
barplot(cbind(as.numeric(Males[,2]), as.numeric(Males[,3]))~Males[,1], beside=TRUE, xlab="")
barplot(cbind(as.numeric(Females[,2]), as.numeric(Females[,3]))~Females[,1], beside=TRUE, xlab="")
dev.off()
...
plot(1:length(Int), as.numeric(Males[Int,4]), type='l', xaxt="n", xlab="", ylim=c(0, 100), lwd=2, col="orange", ylab="% change from 2020 to 2020")
axis(1, at=1:length(Int), labels=Int, las=2)
lines(1:length(Int), as.numeric(Females[Int,4]), lwd=2, col="green")

barplot(Dat3[,2], names.arg=Dat3[,1], las=2)
